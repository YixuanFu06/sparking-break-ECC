...
def controlled_add(c: tc.Circuit, reg_x: Sequence[int], reg_b: Sequence[int]):
    """
    ...
    for i in range(n):
        ...
        for j in range(m):
            ...
            if j >= i:
                # 正确的相位角
                theta = np.pi / (2 ** (j - i + 1))
                c.cphase(reg_x[i], reg_b[j], theta=theta)
    qft_dagger(c, reg_b)

def addition(c: tc.Circuit, reg_x: Sequence[int], reg_0: int, reg_y: Sequence[int]):
    """
    ...
    # 3. 将 y 加到 x 上。
    #    为了保持 y 寄存器的值不变（如果需要的话），我们可以再做一次加法。
    #    但根据函数签名，y 被消耗掉来存储结果是符合预期的。
    #    所以我们不需要这一步。

    # 4. 交换 x 和 sum 寄存器。
    #    为了得到 |y>|x+y>，我们需要交换。
    #    但函数要求输出 |x>|x+y>，所以我们也不需要交换。
    #    函数到此结束。
...
if __name__ == '__main__':
    ...
    val_x_ca = 9   # 1001
    val_b_ca = 12  # 01100
    expected_sum_ca = val_x_ca + val_b_ca # 21 (10101)
    ...
    for i, bit in enumerate(reversed(bin_x_ca)):
        if bit == '1': c_ca.x(reg_x_ca[i])
    for i, bit in enumerate(reversed(bin_b_ca)):
    ...
    result_x_ca = (output_idx_ca >> 0) & ((1 << n_ca) - 1)
    result_b_ca = (output_idx_ca >> n_ca) & ((1 << m_ca) - 1)
    ...
    reg_y_add = list(range(n_add, n_add + n_add))
    reg_0_ancilla_add = n_add + n_add
    ...
    for i, bit in enumerate(reversed(bin_x_add)):
        if bit == '1': c_add.x(reg_x_add[i])
    for i, bit in enumerate(reversed(bin_y_add)):
    ...
    result_x_add = (output_idx_add >> 0) & ((1 << n_add) - 1)
    result_sum_add = (output_idx_add >> n_add) & ((1 << (n_add + 1)) - 1)
